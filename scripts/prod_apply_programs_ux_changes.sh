...
required = {
    ...
    ("competencies", "0001_competence_indicators"),
}
missing = required - applied
assert not missing, (
    "Required previous migrations are not recorded: "
...
assert ProgramPlxImportDraft._meta.db_table == "program_plx_import_draft"
tables = set(connection.introspection.table_names())
assert "program_plx_import_draft" in tables, "program_plx_import_draft table missing"
...
with connection.cursor() as cursor:
    found = set(connection.introspection.get_constraints(cursor, "program_plx_import_draft"))
missing = required_objects - found
assert not missing, f"PLX draft DB objects missing: {sorted(missing)}"
print("Programs UX schema check OK")
'
...
backup_name="programs_ux_changes_$(date +%Y%m%d_%H%M%S).dump"
mkdir -p "$backup_dir"
...
case "$mode" in
    docker)
        ...
        log "Checking database connection and previous migrations"
        ...
        log "Applying programs UX migration"
        docker compose exec -T web sh -c \
            'DJANGO_ENABLE_LOCAL_MIGRATIONS=1 python manage.py migrate programs 0002_plx_import_draft'

        log "Running Django and schema checks"
        docker compose exec -T web sh -c 'DJANGO_ENABLE_LOCAL_MIGRATIONS=1 python manage.py check'
        docker compose exec -T web sh -c 'DJANGO_ENABLE_LOCAL_MIGRATIONS=1 python manage.py check_db_schema --live'
        docker compose exec -T web sh -c \
...
log "Completed programs UX changes. Backup: $backup_dir/$backup_name"
