...
Документ описывает текущее состояние кода в репозитории. Здесь нет обещаний о функциональности, которой в проекте не видно по `settings.py`, `urls.py`, моделям, формам, views, сервисам и шаблонам.
...
Экспорт Word реализован через импорт `docx` в `export/services.py`. Прямого импорта `docxtpl` в коде сейчас нет, хотя пакет указан в зависимостях.

## Структура проекта

```text
assessment_rinh/
├─ assessment_rinh/     # настройки проекта и корневые urls
...
Если связка программы и дисциплины не найдена или по фильтрам нет заданий, view возвращает форму с ошибкой.
...
Текущее ограничение: большинство пользовательских CRUD-view не используют `PermissionRequiredMixin`. Рабочее место преподавателя и create/update/delete заданий требуют авторизации, но единая проверка permissions для всех разделов в коде не реализована.

## База данных

Настройки подключения находятся в `assessment_rinh/settings.py`.
...
- host: `localhost`;
- port: `5432`;
- пароль задан прямо в `settings.py`.
...
- `DEBUG = True`, `ALLOWED_HOSTS = ['*']` и `SECRET_KEY` находятся в `settings.py`; это конфигурация разработки, не production.
- Параметры подключения к БД жестко заданы в `settings.py`, переменные окружения не используются.
