...
4. Запустить `python manage.py check_db_schema --live`.
5. Запустить `python manage.py check`.
6. Запустить `python manage.py seed_initial_data`.
7. Запустить `python manage.py setup_teacher_group`.
8. Создать superuser через `python manage.py createsuperuser`, если его нет.
9. Запустить `collectstatic`.
10. Запустить gunicorn или `docker compose up -d`.
11. Проверить backup scheduler и выполнить тестовый backup/restore на отдельной базе или стенде.
